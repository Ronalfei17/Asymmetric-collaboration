...
export function writeQuickLightingValuesToUI(
    state,
    fixture
) {
    if (!state) {
...
export function updatePanTiltUI() {
    const panSlider =
        getElement('panSlider');
...
export function setupQuickLightingListeners(
    ...
    if (
        ...
        resetAnglesBtn.addEventListener(
            'click',
            () => {
                panSlider.value = 0;
                tiltSlider.value = 0;
